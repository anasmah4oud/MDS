...
import { motion, useScroll, useSpring, AnimatePresence, useInView, useMotionValue, animate } from 'framer-motion';
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden" dir="rtl">
      ...
      <div className="bg-blue-600 py-6 overflow-hidden my-10 border-y-4 border-white/20">
        <div className="flex min-w-[200%] gap-12 animate-[marquee_18s_linear_infinite]" style={{ willChange: 'transform' }}>
          {[...Array(2)].map((_, j) =>
            Array(12).fill('( وما توفيقي إلا بالله )').map((text, i) => (
              <span
                key={`${j}-${i}`}
                className="text-white text-3xl md:text-4xl font-black tracking-[0.45em] whitespace-nowrap"
              >
                {text}
              </span>
            ))
      ...
      <section className="relative py-32 bg-slate-950 text-white overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 relative z-10">
          <div className="text-center mb-14">
            <div className="relative inline-flex items-center justify-center w-28 h-28 mx-auto rounded-full bg-white/10 border border-white/10 shadow-[0_0_60px_rgba(59,130,246,0.18)]">
      ...
      <footer className="relative bg-slate-950 text-white pt-20 pb-10 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-12">
            ...
            <div className="text-center">
              ...
              <div className="flex flex-col gap-4 text-slate-400">
                <Link to="/support" className="footer-link-underline hover:text-blue-400 transition-colors">الدعم الفني</Link>
                <Link to="/contact" className="footer-link-underline hover:text-blue-400 transition-colors">تواصل معنا</Link>
                <Link to="/login" className="footer-link-underline hover:text-blue-400 transition-colors">تسجيل الدخول</Link>
              </div>
            </div>
          ...
          <div className="flex justify-center md:justify-end gap-4 mb-8">
            <a href="https://www.youtube.com" target="_blank" rel="noreferrer" className="p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
              <Youtube size={20} />
            </a>
            <a href="https://www.facebook.com" target="_blank" rel="noreferrer" className="p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
              <Facebook size={20} />
            </a>
            <a href="https://t.me" target="_blank" rel="noreferrer" className="p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
              <Send size={20} />
            </a>
          </div>
...
function FeatureItem({ icon, title, desc, fromLeft }: { icon: React.ReactNode, title: string, desc: string, fromLeft: boolean }) {
  return (
    <motion.div
      ...
      className="glass-effect p-8 rounded-[2rem] flex flex-col items-center text-center transition-all"
      style={{ willChange: 'transform, box-shadow' }}
    >
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-white bg-gradient-to-br from-blue-500 to-violet-500 text-2xl">
        {icon}
      </div>
...
function GradeCard({ grade, title, img }: { grade: number, title: string, img: string }) {
  ...
  return (
    <Link to="/login" className="group">
      <div className="rounded-[2.25rem] p-[1px] bg-gradient-to-r from-orange-400 via-fuchsia-500 to-cyan-500">
        <motion.div
          whileHover={{ scale: 1.02 }}
          className="rounded-[2.15rem] bg-white overflow-hidden shadow-2xl border border-white/10"
          onMouseMove={handleMouseMove}
          onMouseLeave={handleMouseLeave}
          style={{ transform: `perspective(1000px) rotateY(${tilt.x}deg) rotateX(${tilt.y}deg)`, willChange: 'transform' }}
        >
...
function MotivationCard({ text }: { text: string }) {
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60 p-6 backdrop-blur-xl shadow-[0_20px_80px_rgba(15,23,42,0.35)] transition-all"
    >
      ...
      <div className="relative flex items-start gap-4">
        ...
        <span className="relative text-lg font-semibold text-white">{text}</span>
      </div>
    </motion.div>
  );
}
...
function StatCard({ value, label, accent, suffix }: { value: number, label: string, accent: string, suffix?: string }) {
  return (
    ...
    >
      <div className={`inline-flex px-4 py-2 rounded-full bg-gradient-to-r ${accent} text-white text-sm font-semibold mb-4`}>{label}</div>
      <div className="text-5xl md:text-6xl font-black text-slate-900 mb-2">{value}{suffix ?? ''}</div>
