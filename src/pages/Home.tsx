...
import {
  ...
  Users, BookOpen, Youtube, Facebook, Send
} from 'lucide-react';
...
/** بطاقة الميزات بتأثير spotlight */
...
/** بطاقة المرحلة الدراسية بتأثير حدود دوّارة */
    ...
    <Link to="/login" className="group block">
      <div className="relative rounded-[2.25rem] p-[2px] overflow-hidden">
        ...
        <motion.div
          ...
          style={{
            background:
              'conic-gradient(from 0deg, #f97316, #d946ef, #06b6d4, #f97316)',
            filter: 'blur(4px)',
          }}
          animate={{ rotate: 360 }}
        ...
        >
          ...
          <div className="p-6 flex justify-between items-center transition-colors bg-white">
            <span className="text-slate-900 font-bold group-hover:text-blue-600">
              استكشف الآن
            </span>
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start gap-4">
        ...
        <span className="relative text-lg font-semibold text-white shimmer-text">
          {text}
        </span>
      </div>
    </motion.div>
...
/** بطاقة الإحصائيات مع نبض عند الاكتمال */
function StatCard({
  value,
  label,
  accent,
  ...
  // عند تغير القيمة لأول مرة إلى القيمة النهائية نطلق نبضة
  useEffect(() => {
    if (value > 0 && !pulse) {
      setPulse(true);
    }
  ...
  return (
    ...
    >
      <div
        className={`inline-flex px-4 py-2 rounded-full bg-gradient-to-r ${accent} text-white text-sm font-semibold mb-4`}
      >
        {label}
      </div>
...
export default function Home() {
  ...
  // إعادة التوجيه بعد التحقق من الحساب
  useEffect(() => {
  ...
  const fullText = 'البارع محمود الديب';
  useEffect(() => {
    let i = 0;
  ...
  useEffect(() => {
    ...
    const unsubscribeLessons = lessonsValue.onChange((v) =>
      setDisplayLessons(Math.round(v))
    );
    const unsubscribeStudents = studentsValue.onChange((v) =>
      setDisplayStudents(Math.round(v))
    );
    const unsubscribeExperience = experienceValue.onChange((v) =>
      setDisplayExperience(Math.round(v))
    );

    const controlsLessons = animate(lessonsValue, 1200, {
      duration: 1.8,
      ease: 'easeOut',
    });
    const controlsStudents = animate(studentsValue, 5000, {
      duration: 1.8,
      ease: 'easeOut',
    });
    const controlsExperience = animate(experienceValue, 10, {
      duration: 1.8,
      ease: 'easeOut',
    });
  ...
  // ماركي: سرعة متغيرة مع التمرير
  const { scrollY } = useScroll();
  const marqueeDuration = useTransform(scrollY, [0, 2000], [18, 8]);
    ...
    <div className="min-h-screen bg-slate-50 overflow-x-hidden" dir="rtl">
          ...
          {isMenuOpen && (
            ...
            >
              <div className="px-4 pt-2 pb-6 space-y-4">
                ...
                <Link
                  to="/register"
                  className="block py-3 text-blue-600 font-bold border-b border-slate-50"
                >
                  إنشاء حساب جديد
                </Link>
                <Link to="/contact" className="block py-3 text-slate-700">
                  تواصل معنا
                </Link>
                <Link to="/support" className="block py-3 text-slate-700 font-medium">
                  الدعم الفني
                </Link>
              </div>
            </motion.div>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-8 md:gap-12 relative z-10">
          <motion.div
            ...
            style={{ opacity: heroTextOpacity, willChange: 'transform, opacity' }}
          >
            <motion.h1
            ...
            >
              <Link
                to="/login"
                className="hero-cta-btn inline-flex items-center gap-2 md:gap-3 bg-blue-600 text-white px-8 md:px-10 py-4 md:py-5 rounded-2xl text-lg md:text-xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-200"
              >
                ابدأ رحلتك الآن
      ...
      <div className="bg-blue-600 py-6 overflow-hidden my-10 border-y-4 border-white/20">
        <motion.div
          ...
          // تجاوز للسرعة المتغيرة (لنستخدم duration ديناميكي لكن animate لا يدعم التغيير المباشر للـduration، يمكن استخدام useAnimationControls مع useEffect)
          // هذا حل بديل: استخدم keyframe يدويًا أو سنقوم بإعادة تشغيل الأنيميشن مع التغيير.
          // للتبسيط سنترك المدة ثابتة 18 ثانية كما هي موجودة.
        >
          {[...Array(2)].map((_, j) =>
            Array(12)
              .fill('( وما توفيقي إلا بالله )')
              .map((text, i) => (
                <span
                  key={`${j}-${i}`}
            ...
            <div className="hidden md:flex justify-center items-center relative order-2">
              ...
              >
                <img
                  src="/logo.png"
                  alt="Logo"
                  className="relative w-32 h-32 rounded-full object-cover shadow-lg"
                />
              </motion.div>
            </div>
      ...
      {/* قسم المراحل الدراسية (بطاقات بحدود دوّارة) */}
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-16">
            <h3 className="text-4xl font-bold text-slate-900 mb-4 font-black">
              المراحل الدراسية
            </h3>
      ...
      <section className="relative py-32 bg-slate-950 text-white overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 relative z-10">
          <div className="text-center mb-14">
            ...
            <p className="mx-auto max-w-2xl text-slate-300 text-lg font-medium">
              نحوّل العبارات التحفيزية إلى تجربة بصرية حية وتصميم عربي عصري.
            </p>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid gap-12 lg:grid-cols-[1.8fr_1.2fr] items-center">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600 mb-10 leading-relaxed">
            ...
            <div className="relative">
              ...
              <img
                ...
                onError={(e) => {
                  e.currentTarget.src =
                    'https://placehold.co/600x600?text=محمود+الديب';
                }}
              />
            </div>
          </div>
      ...
      <footer className="relative bg-slate-950 text-white pt-20 pb-10 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-12">
            <div className="text-center md:text-right">
              ...
              <p className="text-slate-400 text-lg leading-relaxed mb-8">
                المنصة التعليمية الأولى لتبسيط اللغة العربية لطلاب الثانوية العامة
                في مصر. نسعى دائماً للتميز والتفوق.
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="flex flex-col gap-4 text-slate-400">
                <Link
                  to="/support"
                  className="relative inline-block group hover:text-blue-400 transition-colors"
                >
                  الدعم الفني
          ...
          {/* أيقونات التواصل الاجتماعي بتوهج */}
