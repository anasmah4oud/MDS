...
export default function VideoView() {
          ...
          events: {
            ...
            'onStateChange': (event: any) => {
              if (event.data === window.YT.PlayerState.PLAYING) {
                ...
                setIsPlaying(false);
                clearInterval(timeIntervalRef.current);
                setShowControls(true); // إبقاء عناصر التحكم ظاهرة عند إيقاف المحاضرة مؤقتاً
              }
            }
          }
        });
  ...
  const startTrackingTime = () => {
    ...
    }, 200); // تحديث سريع جداً لضمان سلاسة حركة التلوين البصري مع حركة الـ Thumb
  };
  ...
  const resetControlsTimeout = () => {
    ...
    if (isPlaying) {
      controlsTimeoutRef.current = setTimeout(() => {
        ...
        setShowRatesMenu(false);
      }, 2000); // إخفاء عناصر التحكم بعد 3.5 ثانية من الثبات لرؤية سينمائية كاملة
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans relative flex flex-col antialiased overflow-x-hidden" dir="rtl">
      ...
      <main className="flex-1 flex flex-col lg:flex-row max-w-[1450px] w-full mx-auto p-4 md:p-6 gap-6 h-auto lg:h-[calc(100vh-6rem)] overflow-hidden">
        ...
        >
          ...
          <AnimatePresence>
            {!isVideoStarted && (
              ...
              >
                ...
                <h3 className="text-white font-black text-sm md:text-base mt-5">اضغط للبدء</h3>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {/* إطار الفيديو ذو الأبعاد الكاملة والنقية 100% */}
          {isVideoStarted && (
          ...
          {isVideoStarted && (
            <div className={`barie-floating-controls-panel ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
              ...
              {/* شريط الـ Time Line الاحترافي بالأزرق الفاخر المتوافق مع الـ RTL برمجياً */}
              ...
              {/* أزرار التشغيل، التقديم، وقائمة سرعات التشغيل ومفتاح العرض الكامل */}
          ...
          {/* الطبقة الحركية الذكية للعلامة المائية لتأمين المحتوى */}
        ...
        {/* الشريط الجانبي الفاخر لمعلومات المحاضرة */}
