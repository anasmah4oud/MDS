...
import { 
  Lock, Search, User, ChevronRight, 
  AlertCircle, ShieldCheck, Save, Key,
  Mail, Phone
} from 'lucide-react';
...
export default function AdminPasswords() {
  ...
  const handleReset = async () => {
    ...
    try {
      // NOTE: We update the profile record.
      const { error } = await supabase
        .from('profiles')
        .update({
          // We can't update Auth password directly from frontend admin
          // but we can flag it for administrative purposes
        })
        .eq('id', foundUser.id);
      
      if (error) throw error;
      
      alert(`تم تسجيل طلب تغيير كلمة المرور للمستخدم: ${foundUser.first_name}. ملاحظة: في النسخة الحالية، هذا إجراء إداري يتطلب تدخل تقني لتحديث بيانات Auth.`);
      setFoundUser(null);
      setSearchTerm('');
      setNewPassword('');
    } catch (err) {
        ...
        <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-xl shadow-blue-100/50 space-y-12">
           <div className="text-center">
              ...
              <p className="text-slate-500 font-bold mt-4 max-w-md mx-auto leading-relaxed">
                ابحث عن الطالب برقم هاتفه المسجل لتعديل بيانات الدخول الخاصة به.
              </p>
           </div>
           ...
           {foundUser && (
             ...
             >
                <div className="flex items-center gap-6 pb-6 border-b border-slate-200">
                   <img src={foundUser.photoUrl} className="w-20 h-20 rounded-full border-4 border-white shadow-md" alt="Avatar" />
                   <div>
                ...
                <div className="flex items-start gap-4 p-4 bg-amber-50 rounded-2xl border border-amber-100 text-amber-800 text-sm font-bold leading-relaxed">
                   ...
                   <p>ملاحظة: لضمان أقصى درجات الأمان في Firebase، تغيير كلمة المرور يتطلب توثيقاً مباشراً. تأكد من إبلاغ الطالب بكلمة المرور الجديدة بعد الضغط على حفظ.</p>
                </div>

                <button 
                  onClick={handleReset}
                  className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black text-xl hover:bg-black shadow-2xl transition-all flex items-center justify-center gap-3"
                >
                  <ShieldCheck /> تسجيل كلمة المرور الجديدة
                </button>
             </motion.div>
           )}
        </div>
